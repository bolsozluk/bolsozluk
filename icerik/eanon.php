...
//echo "anonimleştirme sistemi geçici olarak devre dışıdır.";
//die;
...
if ($id and $sira) {
	...
	$sebep = strtolower($sebep);
		...
		if (!$ucur) {
		$sorgu = "UPDATE mesajlar SET `yazar` = 'anonim' WHERE id='$id'";
		mysql_query($sorgu);		
		$sorgu = "UPDATE oylar SET entry_sahibi='anonim' WHERE entry_id='$id'";
		mysql_query($sorgu);		
